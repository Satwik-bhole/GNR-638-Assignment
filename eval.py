...
def main(test_path, load_path):
    ...
    # Requirement: Measure and report dataset loading time [cite: 39, 40, 102]
    load_start = time.time()
    dataset = nn.load_dataset(test_path)
    num_classes = len(dataset.class_names)
    print(f"Dataset Loading Time: {time.time() - load_start:.4f} seconds")

    # Architecture must match train.py exactly [cite: 42-46, 99]
    ...
    flat = nn.Flatten()
    fc = nn.Linear(128 * 4 * 4, num_classes)
    
    layers = [conv1, relu1, pool1, conv2, relu2, pool2, conv3, relu3, pool3, flat, fc]

    # Requirement: Report total trainable parameters [cite: 47, 100]
    ...
    for i in range(0, total, BATCH_SIZE):
        ...
        # Requirement: Print MACs during evaluation [cite: 48, 101]
    ...
    # Requirement: Evaluation metrics [cite: 103]
    acc = correct / total
